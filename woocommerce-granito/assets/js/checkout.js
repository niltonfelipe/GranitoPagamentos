...
(function( $ ) {
	...
	$( function() {
		...
		function isCheckoutInvalid( evt ) {
			...
			if ( 0 < $( 'input[name=granito_checkout_token]' ).length ) {
				return true;
			}
		...
		function openCheckout() {
			...
			checkout = new GranitoCheckout.Checkout({
				...
				success: function( data ) {
					...
					if ( 'no' === wcGranitoParams.register_refused_order ) {
						// Remove any old token input.
						$( 'input[name=granito_checkout_token]', form ).remove();

						// Add the token input.
						form.append( $( '<input name="granito_checkout_token" type="hidden" />' ).val( data.token ) );
					} else {
						// Remove any old card_hash and installments input.
						$( 'input[name=granito_card_hash]', form ).remove();
						$( 'input[name=granito_installments]', form ).remove();

						// Add the token input.
						form.append( $( '<input name="granito_card_hash" type="hidden" />' ).val( data.card_hash ) );
						form.append( $( '<input name="granito_installments" type="hidden" />' ).val( data.installments ) );
					}
			...
			if ( wcGranitoParams.checkoutPayPage ) {
				customer = wcGranitoParams.customerFields;
			} else {
				customer = getCustomerFields( form );
			}

			const shouldCreateToken = 'yes' === wcGranitoParams.register_refused_order ? 'false' : 'true';
			...
			params = $.extend({}, {
				...
				interestRate:     wcGranitoParams.interestRate,
				maxInstallments:  inline_data.data( 'max_installment' ),
				freeInstallments: wcGranitoParams.freeInstallments,
				postbackUrl:      wcGranitoParams.postbackUrl,
				uiColor:          wcGranitoParams.uiColor
			}, customer );
